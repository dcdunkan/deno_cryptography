...
export default class AES {
  ...
  encrypt(_message: string | Uint32Array | Uint8Array) {
    ...
    for (let i = 0; i < rounds; i++) {
      a2 = T1[a >>> 24] ^ T2[(b >> 16) & 255] ^ T3[(c >> 8) & 255] ^
        T4[d & 255] ^ this.encKey[k];
      b2 = T1[b >>> 24] ^ T2[(c >> 16) & 255] ^ T3[(d >> 8) & 255] ^
        T4[a & 255] ^ this.encKey[k + 1];
      c2 = T1[c >>> 24] ^ T2[(d >> 16) & 255] ^ T3[(a >> 8) & 255] ^
        T4[b & 255] ^ this.encKey[k + 2];
      d = T1[d >>> 24] ^ T2[(a >> 16) & 255] ^ T3[(b >> 8) & 255] ^
        T4[c & 255] ^ this.encKey[k + 3];
      a = a2;
      b = b2;
      c = c2;
      k += 4;
  ...
  decrypt(_message: string | Uint32Array | Uint8Array) {
    ...
    // Inner rounds.  Cribbed from OpenSSL.
    for (let i = 0; i < rounds; i++) {
      a2 = T5[a >>> 24] ^ T6[(b >> 16) & 255] ^ T7[(c >> 8) & 255] ^
        T8[d & 255] ^ this.decKey[k];
      b2 = T5[b >>> 24] ^ T6[(c >> 16) & 255] ^ T7[(d >> 8) & 255] ^
        T8[a & 255] ^ this.decKey[k + 1];
      c2 = T5[c >>> 24] ^ T6[(d >> 16) & 255] ^ T7[(a >> 8) & 255] ^
        T8[b & 255] ^ this.decKey[k + 2];
      d = T5[d >>> 24] ^ T6[(a >> 16) & 255] ^ T7[(b >> 8) & 255] ^
        T8[c & 255] ^ this.decKey[k + 3];
      a = a2;
      b = b2;
      c = c2;
      k += 4;
