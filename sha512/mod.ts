...
function round(state: Uint32Array, data: Uint32Array) {
  ...
  for (let i = 32; i < 160; i += 2) {
    ...
    words[i] = t1hi + words[i - 14] + t2hi + words[i - 32] +
      ((lo / 0x100000000) >>> 0);
    words[i + 1] = lo;
  }
  ...
  for (let i = 0; i < 160; i += 2) {
    ...
    const t1hi =
      (hhi + s1hi + chhi + _k[i] + words[i] + ((t1lo / 0x100000000) >>> 0)) >>>
      0;
    t1lo >>>= 0;
 ...
 * Repeatable part
 */
function finish(len: number, buf: Uint32Array, state: Uint32Array, offset = 0) {
